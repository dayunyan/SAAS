...
def make_cam(img_dir, to_path):
    ...
    model.load_state_dict(torch.load(MODE.model_woPerloss_path, map_location=device))
    model.to(device)
    model.eval()
    # print(to_path)
...
if __name__ == "__main__":
    if not os.path.exists(MODE.train_woPerloss_psm):
        os.makedirs(MODE.train_woPerloss_psm)
    if not os.path.exists(MODE.val_woPerloss_psm):
        os.makedirs(MODE.val_woPerloss_psm)
    if not os.path.exists(MODE.test_woPerloss_psm):
        os.makedirs(MODE.test_woPerloss_psm)
    if not os.path.exists(MODE.aux_woPerloss_psm):
        os.makedirs(MODE.aux_woPerloss_psm)

    make_cam(os.path.join(MODE.train_dir, "residential"), MODE.train_woPerloss_psm)
    make_cam(os.path.join(MODE.val_dir, "residential"), MODE.val_woPerloss_psm)
    make_cam(MODE.test_dir, MODE.test_woPerloss_psm)
    make_cam(MODE.aux_postive_dir, MODE.aux_woPerloss_psm)

    if not os.path.exists(MODE.full_woPerloss_train):
        os.mkdir(MODE.full_woPerloss_train)
    else:
        shutil.rmtree(MODE.full_woPerloss_train)
        os.mkdir(MODE.full_woPerloss_train)

    if not os.path.exists(MODE.full_woPerloss_train_gt):
        os.mkdir(MODE.full_woPerloss_train_gt)
    else:
        shutil.rmtree(MODE.full_woPerloss_train_gt)
        os.mkdir(MODE.full_woPerloss_train_gt)

    cat_full(MODE.train_dir + "residential/", MODE.full_woPerloss_train)
    cat_full(MODE.aux_postive_dir, MODE.full_woPerloss_train)
    cat_full(MODE.train_woPerloss_psm, MODE.full_woPerloss_train_gt)
    cat_full(MODE.aux_woPerloss_psm, MODE.full_woPerloss_train_gt)
