...
def train_haze():
    ...
    for epoch in range(num_epoch):
        ...
        for i, [data, labels] in enumerate(train_dataloader):
            ...
            loss = criterion[0](
                outputs_haz, labels
            )  # + a*criterion[1](cams_cln, cams_haz) + b*criterion[2](outputs_cln, outputs_haz) + alpha*criterion[3](cams_cln, cams_haz, inputs_haz)
...
def test_haze():
    ...
    model.load_state_dict(
        torch.load(MODE.model_nonconsist_path, map_location=args.device)
    )
    # print(model)
    model.eval()
