...
from models.loser import CAMRefineLoss, BackwardLoss
...
def train_haze():
    ...
    criterion = [
        ...
        ClsConsis_Loss,
        CAMRefineLoss(),
        BackwardLoss(smodel, device=device),
    ]
    scheduler = optim.lr_scheduler.StepLR(optimizer, 5, 0.5)

    # Set CAM extractor
    tcam_extractor = class_activation_map(tmodel, target_layer="cls")
    scam_extractor = class_activation_map(smodel, target_layer="cls")
    ...
    for epoch in range(num_epoch):
        ...
        for i, [data, labels] in enumerate(train_dataloader):
            ...
            # forward
            _, outputs_cln = smodel(inputs_cln)
            _, outputs_haz = tmodel(inputs_haz)

            # cam
            batch = inputs_cln.size()[0]
            cams_cln = np.zeros((batch, 1, config.weight, config.height))
            cams_haz = np.zeros((batch, 1, config.weight, config.height))
            sactivation_map = scam_extractor(
                outputs_cln.argmax(dim=1).cpu().numpy().tolist(),
                outputs_cln,
                retain_graph=True,
            )
            tactivation_map = tcam_extractor(
                outputs_haz.argmax(dim=1).cpu().numpy().tolist(),
                outputs_haz,
                retain_graph=True,
            )
            for j in range(batch):
                ...
                ssmap = sact_pil_map.resize(
                    (config.weight, config.height), Image.Resampling.LANCZOS
                )
                #                 ssmap = (255 * cmap(np.asarray(ssmap) ** 2)[:, :, :3]).astype(np.uint8)
                cams_cln[j, :, :, :] = np.array(ssmap)

                tact_pil_map = to_pil_image(tactivation_map[0][j, :, :], mode="F")
                #           print(np.array(act_pil_map))
                tsmap = tact_pil_map.resize(
                    (config.weight, config.height), Image.Resampling.LANCZOS
                )
                cams_haz[j, :, :, :] = np.array(tsmap)
            cams_cln = torch.from_numpy(cams_cln).to(device)
            cams_haz = torch.from_numpy(cams_haz).to(device)

            # print(outputs.size())
            # loss
            # print(
            #     f"cross_entropy:{criterion[0](outputs_haz, labels)}, cam:{criterion[1](cams_cln, cams_haz)}, cls:{criterion[2](outputs_cln, outputs_haz)}, camref:{criterion[3](cams_cln, cams_haz, inputs_cln)}"
            # )
            loss = (
                criterion[0](outputs_haz, labels)
                + a * criterion[1](cams_cln, cams_haz)
                + b * criterion[2](outputs_cln, outputs_haz)
                + alpha * criterion[3](cams_cln, cams_haz, inputs_cln)
                + beta * criterion[4](cams_haz, inputs_cln)
            )
            ...
            if i % 10 == 9:
                for j, [data, labels] in enumerate(val_dataloader):
                    ...
                    cams_haz = np.zeros((batch, 1, config.weight, config.height))
                    sactivation_map = scam_extractor(
                        outputs_cln.argmax(dim=1).cpu().numpy().tolist(), outputs_cln
                    )
                    tactivation_map = tcam_extractor(
                        outputs_haz.argmax(dim=1).cpu().numpy().tolist(), outputs_haz
                    )
                    for k in range(batch):
                        #                         print('----val_batch:{}'.format(j))
                        sact_pil_map = to_pil_image(
                            sactivation_map[0][k, :, :], mode="F"
                        )
                        #           print(np.array(act_pil_map))
                        ssmap = sact_pil_map.resize(
                            (config.weight, config.height), Image.Resampling.LANCZOS
                        )
                        cams_cln[k, :, :, :] = np.array(ssmap)

                        tact_pil_map = to_pil_image(
                            tactivation_map[0][k, :, :], mode="F"
                        )
                        #           print(np.array(act_pil_map))
                        tsmap = tact_pil_map.resize(
                            (config.weight, config.height), Image.Resampling.LANCZOS
                        )
                        cams_haz[k, :, :, :] = np.array(tsmap)
                    cams_cln = torch.from_numpy(cams_cln).to(device)
                    cams_haz = torch.from_numpy(cams_haz).to(device)
