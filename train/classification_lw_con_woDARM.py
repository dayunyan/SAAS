...
def train_haze():
    ...
    if args.cls_model == "resnet":
        ...
        tmodel.load_state_dict(
            torch.load(config.CLEAR.pcares_woAtt_path, map_location=device)
        )
    # print(tmodel)
    tmodel.to(device)
    ...
    for epoch in range(num_epoch):
        ...
        for i, [data, labels] in enumerate(train_dataloader):
            ...
            loss = (
                ...
                + alpha * criterion[3](cams_cln, cams_haz, inputs_haz)
            )
    ...
    print("Train finish!")
    # 保存模型
    model_path = MODE.model_woAtt_path
...
def test_haze():
    ...
    #     print(model)
    # model.to(device)
    model.load_state_dict(torch.load(MODE.model_woAtt_path, map_location=args.device))
    # print(model)
    model.eval()
