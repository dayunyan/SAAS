...
MODE = config.NONUNIHAZE #config.HAZE
...
#                     loss = loss_fc(out_haz, labels)+CAM_Loss(out_cln_cam,out_haz_cam)+ClsConsis_Loss(out_cln_fc,out_haz_fc)
    ...
    torch.save(acc_best_wts, MODE.pcares_woPerloss_path)
    print('Model save ok!')
...
def test_haze():
    ...
    model.load_state_dict(torch.load(MODE.pcares_woPerloss_path, map_location='cpu'))
    print(model)
    model.eval()
