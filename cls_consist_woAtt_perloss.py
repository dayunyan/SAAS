...
def train_haze():
    ...
    smodel.load_state_dict(torch.load(config.CLEAR.pcares_woAtt_path, map_location=device))
    smodel.to(device)
    smodel.eval()
    ...
    return_layers = {'layer4':'layer4', 'fc':'fc'}
    for (name, module) in smodel.named_modules():
        if name == return_layers['layer4']:
    ...
    # Set CAM extractor
    tcam_extractor = XGradCAM(tmodel,target_layer='layer4')
    scam_extractor = XGradCAM(smodel,target_layer='layer4')
...
#                     loss = loss_fc(out_haz, labels)+CAM_Loss(out_cln_cam,out_haz_cam)+ClsConsis_Loss(out_cln_fc,out_haz_fc)
    ...
    torch.save(acc_best_wts, MODE.pcares_woAtt_perloss_path)
    print('Model save ok!')
...
def test_haze():
    ...
    model.load_state_dict(torch.load(MODE.pcares_woAtt_perloss_path, map_location='cpu'))
    print(model)
    model.eval()
